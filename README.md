...
#### Options
 ...
 | trustProxy                  | false            | Trust the `X-Forwarded-Prefix` header to compute asset URLs when behind a reverse proxy that strips a path prefix. Only enable this when the server is genuinely behind a proxy that controls this header, since it takes precedence over `indexPrefix`. |
