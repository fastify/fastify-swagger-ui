...
const getForwardedPrefix = require('./get-forwarded-prefix')
...
function fastifySwagger (fastify, opts, done) {
  ...
  fastify.route({
    ...
    ...hooks,
    handler: (req, reply) => {
      const forwardedPrefix = getForwardedPrefix(opts, req.headers)
      reply
        .header('content-type', 'text/html; charset=utf-8')
        .send(indexHtmlContent(req.url, forwardedPrefix)) // trailing slash alters the relative urls generated in the html
    }
  })
  ...
  fastify.route({
    ...
    handler: (req, reply) => {
      const forwardedPrefix = getForwardedPrefix(opts, req.headers)
      const location = req.url.replace(/\/static\/index\.html$/, '/')
      reply.redirect(forwardedPrefix ? `${forwardedPrefix}${location}` : location)
    }
  })
