...
function fastifySwagger (fastify, opts, done) {
  ...
  const hasTransformSwaggerFn = typeof opts.transformSwagger === 'function'
  const shouldCloneSwaggerObject = opts.transformSwaggerClone ?? true
  const transformSwagger = opts.transformSwagger
  fastify.route({
    url: '/json',
    method: 'GET',
    schema: { hide: true },
    ...hooks,
    handler: hasTransformSwaggerFn
      ? shouldCloneSwaggerObject
        ? function (req, reply) {
          reply.send(transformSwagger(rfdc(fastify.swagger()), req, reply))
        }
        : function (req, reply) {
          reply.send(transformSwagger(fastify.swagger()), req, reply)
        }
      : function (req, reply) {
      ...
      ? shouldCloneSwaggerObject
        ? function (req, reply) {
          reply
            ...
            .send(yaml.stringify(transformSwagger(rfdc(fastify.swagger()), req, reply)))
        }
        : function (req, reply) {
          reply
            .type('application/x-yaml')
            .send(yaml.stringify(transformSwagger(fastify.swagger()), req, reply))
        }
      : function (req, reply) {
        reply
          .type('application/x-yaml')
