...
function fastifySwagger (fastify, opts, done) {
  ...
  function schema(schemaOpts) {
    return fastify[opts.decorator ?? 'swagger'](schemaOpts);
  }
    ...
    handler: hasTransformSpecificationFn
      ? shouldCloneSwaggerObject
        ? async function (req, reply) {
          return reply.send(transformSpecification(rfdc(await schema()), req, reply))
        }
        : async function (req, reply) {
          return reply.send(transformSpecification(await schema(), req, reply))
        }
      : async function (_req, reply) {
        return reply.send(await schema())
      }
  })
      ...
      ? shouldCloneSwaggerObject
        ? async function (req, reply) {
          return reply
            ...
            .send(yaml.stringify(transformSpecification(rfdc(await schema()), req, reply)))
        }
        : async function (req, reply) {
          return reply
            .type('application/x-yaml')
            .send(yaml.stringify(transformSpecification(await schema(), req, reply)))
        }
      : async function (_req, reply) {
        return reply
          .type('application/x-yaml')
          .send(await schema({ yaml: true }))
      }
  })
