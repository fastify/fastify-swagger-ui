...
function indexHtml (opts) {
  const indexPrefix = opts.indexPrefix?.replace(/\/$/, '') ?? ''
  return (url, forwardedPrefix) => {
    let routePrefix = opts.prefix
    if (forwardedPrefix) {
      routePrefix = `${forwardedPrefix}${opts.prefix}`
    } else if (indexPrefix) {
      routePrefix = `${indexPrefix}/${opts.prefix.replace(/^\//, '')}`
    }
